...
import { demandApi, type Demand, type DemandListParams } from '@/api/rk/demand'
...
const columns: Column[] = [
  ...
  { prop: 'title', label: '职位名称', minWidth: 150 },
  { prop: 'customerName', label: '客户', minWidth: 120 },
  { prop: 'headcount', label: '人数', width: 80 },
  { prop: 'workLocation', label: '工作地点', minWidth: 120 },
  { prop: 'salaryRange', label: '薪资范围', width: 120 },
...
const searchParams = reactive<DemandListParams>({
  keyWord: '',
  status: undefined,
...
async function loadData() {
  ...
  try {
    ...
    const { list, pagination: pager } = await demandApi.getList(params)
    tableData.value = list || []
    pagination.total = pager?.total || 0
  } catch (e) {
    console.error('Failed to load demand list:', e)
  } finally {
    loading.value = false
  }
}
...
// Selection change
function handleSelectionChange(rows: Demand[]) {
  selectedRows.value = rows
}

// Add new
function handleAdd() {
  router.push('/rk/demand/add')
}

// Edit
function handleEdit(row: Demand) {
  router.push(`/rk/demand/edit/${row.id}`)
}

// View detail
function handleView(row: Demand) {
  router.push(`/rk/demand/detail/${row.id}`)
}
